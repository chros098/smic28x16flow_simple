...
| fp | `pr/scr/02_invs_floorplan.tcl` + `002_floorplan_customize_template.tcl` | floorPlan -r 1.2 0.6 5 5 5 5、3 个固定面积 60000 µm² 宏 keep-out 区域（顶部 300×200、右下角 300×200、左下 L 形 250×160+100×200）、M7/M6 电源环 + M6 条纹 + sroute |
...
- Floorplan：aspect ratio 1.2、core utilization 0.6、margin 5
...
- 若提供 floorplan DEF，`vars(fp_def_file)` 设置后 fp 步骤会自动加载，
   否则使用默认 `floorPlan -r 1.2 0.6 5 5 5 5`。
